...
namespace Tdf
{
    public class TdfLegacyDecoder : ITdfDecoder
    {
        ...
        private bool ReadTdf(Stream stream, ref object? instance, Dictionary<string, FieldInfo> context)
        {
            ...
            {
                throw new InvalidOperationException($"Error reading TDF field '{tdfMember}' (Tag: {tdfMember.Tag}) as {baseType}, field={field?.Name} (Type={field?.FieldType?.FullName}), size={size}: {ex.Message}", ex);
            }
        }
        ...
        private bool ReadTdfStruct(Stream stream, byte size, ref object? instance, FieldInfo? field)
        {
            ...
            // Special case: if field type is string, but we're reading as struct, something is wrong
            // Try to handle it gracefully by reading as string instead
        ...
        {
            // Handle types that cannot be instantiated or shouldn't be created as structs
            // Strings cannot be created with Activator.CreateInstance
            if (type == typeof(string))
            {
                throw new InvalidOperationException($"ReadTdfStruct called with string type. This indicates a TDF structure mismatch - a field expected to be a struct is actually a string. Field definition may be incorrect or client is sending unexpected data.");
            }
            
            // Primitives shouldn't be read as structs (they should use their specific readers)
            if (type.IsPrimitive)
            {
                throw new InvalidOperationException($"ReadTdfStruct called with primitive type: {type.FullName}. This may indicate a TDF structure definition mismatch.");
            }
